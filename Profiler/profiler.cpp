...
void ProfilerInstance::start() { startTime_ = SteadyClock::now(); }

void ProfilerInstance::end() {
  finishTime_ = SteadyClock::now();
  duringTime_ =
      std::chrono::duration_cast<DurationTime>(finishTime_ - startTime_);
}
...
double ProfilerInstance::second() { return duringTime_.count(); }

double ProfilerInstance::milliSecond() { return duringTime_.count() * 1000; }
