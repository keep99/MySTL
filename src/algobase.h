#ifndef TOYSTL_SRC_ALGOBASE_H_
#define TOYSTL_SRC_ALGOBASE_H_
...
OutputIter __copy_d(RandomIter first, RandomIter last, OutputIter result,
                    Distance*) {
...
OutputIter __copy(RandomIter first, RandomIter last, OutputIter result,
...
// copy_backward 具体的实现过程参考
// https://blog.csdn.net/dongyu_1989/article/details/80061944
/******************************************************************************/
// 底层的实现1
template <class BidirectionalIter1, class BidirectionalIter2>
BidirectionalIter2 __copy_backward(BidirectionalIter1 first,
...
BidirectionalIter2 copy_backward(BidirectionalIter1 first,
                                 BidirectionalIter1 last,
                                 BidirectionalIter2 result) {
  return __copy_backward_dispatch<BidirectionalIter1, BidirectionalIter2>()(
      first, last, result);
}
...
toystl::pair<InputIterator, OutputIterator> __copy_n(InputIterator first,
                                                     Size n,
  ...
  return toystl::pair<InputIterator, OutputIterator>(first, result);
}

template <class InputIterator, class Size, class OutputIterator>
toystl::pair<InputIterator, OutputIterator> __copy_n(
    InputIterator first, Size n, OutputIterator result,
    random_access_iterator_tag) {
  auto last = first + n;
  return pair<InputIterator, OutputIterator>(last,
                                             toystl::copy(first, last, result));
}

template <class InuputIterator, class Size, class OutputIterator>
toystl::pair<InuputIterator, OutputIterator> copy_n(InuputIterator first,
                                                    Size n,
                                                    OutputIterator result) {
...
template <class InputIterator, class OutputIterator>
OutputIterator __move(InputIterator first, InputIterator last,
                      OutputIterator result, input_iterator_tag) {
  for (; first != last; ++first, ++result) {
    *result = toystl::move(*first);
  }
...
template <class RandomIter, class OutputIter, class Distance>
OutputIter __move_d(RandomIter first, RandomIter last, OutputIter result,
                    Distance*) {
  for (Distance n = last - first; n > 0; --n, ++result, ++first) {
    *result = *first;
  }
...
T* __move_t(const T* first, const T* last, T* result, false_type) {
  return __move_d(first, last, result, static_cast<ptrdiff_t*>(0));
}
...
template <class BidirectionalIter1, class BidirectionalIter2>
BidirectionalIter2 __move_backward(BidirectionalIter1 first,
                                   BidirectionalIter1 last,
                                   BidirectionalIter2 result,
                                   bidirectional_iterator_tag) {
  while (first != last) {
    // * 和 -- 的优先级相同，结合方向从右到左
    *--result = toystl::move(*--last);
  }
...
BidirectionalIter2 __move_backward_d(RandomIter first, RandomIter last,
                                     BidirectionalIter2 result, Distance*) {
...
template <class T>
T* __move_backward_t(const T* first, const T* last, T* result,
                     toystl::true_type) {
  const size_t n = static_cast<size_t>(last - first);
  if (n != 0) {
    result -= n;
...
T* __move_backward_t(const T* first, const T* last, T* result,
                     toystl::false_type) {
  return __move_backward_d(first, last, result, static_cast<ptrdiff_t*>(0));
}
...
BidirectionalIter2 move_backward(BidirectionalIter1 first,
                                 BidirectionalIter1 last,
                                 BidirectionalIter2 result) {
  return __move_backward_dispatch<BidirectionalIter1, BidirectionalIter2>()(
      first, last, result);
}
...
bool euqal(InputIterator1 frist1, InputIterator1 last1, InputIterator2 first2,
           Compare comp) {
...
toystl::pair<InputIter1, InputIter2> mismatch(InputIter1 first1,
                                              InputIter1 last1,
                                              InputIter2 first2) {
  ...
  return toystl::pair<InputIter1, InputIter2>(first1, first2);
}
...
inline bool lexicographical_compare(const unsigned char* first1,
  ...
  const auto result = memcmp(first1, first2, toystl::min(len1, len2));
  // 若相等，长度较长得比较大
  return result != 0 ? result < 0 : (len1 < len2);
}
}  // namespace toystl

#endif  // TOYSTL_SRC_ALGOBASE_H_