#ifndef TOYSTL_SRC_RB_TREE_H_
#define TOYSTL_SRC_RB_TREE_H_
...
// case4：父节点为红，叔叔节点为 NIL
// 或黑色，父节点为左（右）子节点，当前节点为右（左）子节点，
//        让父节点成为当前节点，再以当前节点为支点左（右）旋
// case5：父节点为红，叔叔节点为 NIL
  ...
  while (x != root && x->parent->color == rb_tree_red) {  // 父节点为红
    if (x->parent == x->parent->parent->left) {  // 父节点为祖父节点的左子节点
      ...
      if (y && y->color == rb_tree_red) {  // 伯父节点存在且为红
        ...
        x = x->parent->parent;  // 但是此时祖父节点为红的话，
        // 祖父节点的父亲可能也是红，可能导致
        // "双红现象" 上溯
      } else {                        // 无叔叔节点或叔叔节点为黑
        if (x->parent->right == x) {  // 如果当前节点为右子节点
          // case4：父节点为红，叔叔节点为 NIL
          // 或黑色，父节点为左子节点，当前节点为右子节点
          x = x->parent;
...
// 删除节点后
// 使 rb_tree 平衡，z 为要删除的节点，root 为根节点，leftmost
// 为最小的节点，rightmost 为最大的节点
inline rb_tree_node_base* rb_tree_rebalance_for_erase(
  ...
  // 2、z 只有一个孩子。那么将 z 父节点的相应儿子指针指向 z 的独生子，删除 z
  // 节点即可。
  else {
    x_parent = y->parent;
    if (x) {
  ...
  // 如果删除的节点为红色，树的性质没有被破坏，否则按照下面情况进行调整（以 x
  // 为左子节点为例）：

  // 以下的情况，删除的节点为黑色。双黑修正。
  // case1
  // ：兄弟节点为红色，令父节点为红，兄弟节点为黑，进行左（右）旋，继续处理
  // case2 ：兄弟节点为黑色，且两个子节点都为黑色或者
  // NIL，令兄弟父节点为红，父节点成为当前节点，继续处理
  // case3 ：兄弟节点为黑色，左子节点为红色或者 NIL，右子节点为黑色或 NIL
  //         令兄弟节点为红，兄弟节点的左子节点为黑，以兄弟节点为支点右（旋），继续处理
  // case4 ：兄弟节点为黑色，两个右子节点都为红色
  //         令兄弟节点为父节点的颜色，父节点为黑色，兄弟节点的右子节点为黑色，以父节点
  //         为支点左（右）旋，树的性质调整完毕，算法结束
      ...
      if (x == x_parent->left) {  // x 为左子节点
        ...
        if ((x_brother->left == nullptr ||
          ...
          if (x_brother->right == nullptr ||
              x_brother->right->color != rb_tree_red) {
            // case3：兄弟节点为黑色，左子节点为红色或者 NIL，右子节点为黑色或
            // NIL
            if (x_brother->left != nullptr) {
              x_brother->left->color = rb_tree_black;
            }
...
class rb_tree {
 ...
 protected:
  ...
  static const Key& getKey(base_ptr x) { return KeyOfValue()(value(x)); }
...
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::insert_equal(
    const value_type& v) {
  link_type y = header;
...
template <class Key, class Value, class KeyOfValue, class Compare,
          class Allocator>
void rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::insert_equal(
...
pair<typename rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::iterator,
     bool>
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::insert_unique(
    const value_type& v) {
  link_type y = header;
  link_type x = root();
  bool comp = true;
  ...
  if (key_compare(getKey(j.node),
                  KeyOfValue()(v)))  // 判断这二者的键值是否相等。key_compare
                                     // 为真，说明 getKey(j.node) >
                                     // KeyOfValue()(v)
...
template <class Key, class Value, class KeyOfValue, class Compare,
          class Allocator>
void rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::insert_unique(
    const_iterator first, const_iterator last) {
  for (; first != last; ++first) {
    insert_unique(*first);
  }
...
template <class Key, class Value, class KeyOfValue, class Compare,
          class Allocator>
typename rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::iterator
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::insert(
    base_ptr x, base_ptr y, const value_type& v) {
  link_type pos = static_cast<link_type>(x);         // 新值插入点
  link_type pos_parent = static_cast<link_type>(y);  // 新值插入点的父节点
  link_type z;                                       // 新值插入的节点

  // 如果新值插入点的父节点是 header 或者 新值插入点是空节点 或者 v 对应的键值
  // 比 新插入点的父节点的键值要小
  if (pos_parent == header || pos != nullptr ||
      key_compare(KeyOfValue()(v), getKey(pos_parent))) {
    z = create_node(v);
...
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::copy(link_type x,
                                                          link_type y) {
...
template <class Key, class Value, class KeyOfValue, class Compare,
          class Allocator>
void rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::erase(link_type x) {
  while (x != nullptr) {
    erase(right(x));
    link_type y = left(x);
    destroy_node(x);
...
template <class Key, class Value, class KeyOfValue, class Compare,
          class Allocator>
void rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::erase(
    iterator position) {
...
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::erase(const key_type& x) {
...
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::find(
...
template <class Key, class Value, class KeyOfValue, class Compare,
          class Allocator>
typename rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::size_type
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::count(
    const key_type& k) const {
...
template <class Key, class Value, class KeyOfValue, class Compare,
          class Allocator>
typename rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::const_iterator
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::lower_bound(
    const Key& k) const {
  link_type y = header;
  link_type x = root();
...
template <class Key, class Value, class KeyOfValue, class Compare,
          class Allocator>
typename rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::const_iterator
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::upper_bound(
    const Key& k) const {
  link_type y = header;
  link_type x = root();
...
template <class Key, class Value, class KeyOfValue, class Compare,
          class Allocator>
pair<typename rb_tree<Key, Value, KeyOfValue, Compare,
                      Allocator>::const_iterator,
     typename rb_tree<Key, Value, KeyOfValue, Compare,
                      Allocator>::const_iterator>
rb_tree<Key, Value, KeyOfValue, Compare, Allocator>::equal_range(
    const Key& key) const {
...
inline bool operator==(
    const rb_tree<Key, Value, KeyOfValue, Compare, Allocator>& x,
...
inline bool operator<(
    const rb_tree<Key, Value, KeyOfValue, Compare, Allocator>& x,
    const rb_tree<Key, Value, KeyOfValue, Compare, Allocator>& y) {
  return toystl::lexicographical_compare(x.begin(), x.end(), y.begin(),
                                         y.end());
}
