#ifndef TOYSTL_SRC_HEAP_H_
#define TOYSTL_SRC_HEAP_H_
...
void push_heap_d(RandomIter first, RandomIter last, Distance*) {
  toystl::push_heap_aux(first, (last - first) - 1, static_cast<Distance>(0),
                        *(last - 1));
}
...
/******************************************************************************/
// 分为下溯和上溯两个过程
// 下溯是把左右子节点中更大的值往上搬，但是没有和那个被 堆顶覆盖的 value
// 进行比较
...
void adjust_heap(RandomIter first, Distance holeIndex, Distance len, T value) {
  ...
  while (rchild < len) {
    if (*(first + rchild) < *(first + rchild - 1)) {
      // 比较洞节点的左右两个子节点的值，找出较大的那个，以 rchild
      // 代表较大的子节点
      rchild--;
    }
...
void pop_heap(RandomIter first, RandomIter last) {
  toystl::pop_heap_aux(first, last - 1, last - 1, *(last - 1),
                       distance_type(first));
}
