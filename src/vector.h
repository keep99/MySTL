#ifndef TOYSTL_SRC_VECTOR_H_
#define TOYSTL_SRC_VECTOR_H_
...
class vector {
 ...
 public:
  ...
  // 所以这里的效果就是 将这个对象的 end finish end 赋值为 other 的，然后将
  // other 的 三个成员 置为 nullptr。
  // vector(const vector&& other) noexcept: vector() {
  //     swap(other);
  // }
  // 以上的实现不好，改为下面的实现
  vector(vector&& other) noexcept : start_(other.start_),
  ...
  ~vector() {
    ...
    // }
    // 先析构，后回收内存。在
    // finish_（包含finish_）之后的位置没有元素，只需要回收内存即可
  ...
  void insert_dispatch(const_iterator position, Integer n, Integer value,
                       true_type) {
    fill_insert(const_cast<iterator>(position), (size_type)n, (T)value);
  }

  template <class InputIterator>
  void insert_dispatch(const_iterator position, InputIterator first,
  ...
  // 保持类型信息是一个两阶段的过程;
  // 1、首先为了保持实参中的类型信息，必须将 emplace
  // 中的函数参数定义为模板类型参数的右值引用
  ...
  void push_back(value_type&& value) { emplace_back(toystl::move(value)); }
 ...
 private:
  ...
  //     fill_initialize(static_cast<size_type>(first),
...
template <class T, class Alloc>
template <class ForwardIterator>
void vector<T, Alloc>::range_insert(iterator pos, ForwardIterator first,
                                    ForwardIterator last,
                                    forward_iterator_tag) {
  if (first != last) {
