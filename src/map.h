#ifndef TOYSTL_SRC_MAP_H_
#define TOYSTL_SRC_MAP_H_
...
namespace toystl {
// map
template <class Key, class T, class Compare = toystl::less<Key>,
...
class map {
 ...
 private:
  using rb_tree_type =
      toystl::rb_tree<key_type, value_type, toystl::selectfirst<value_type>,
                      Compare, Allocator>;
...
template <class K1, class T1, class C1, class A1>
bool operator==(const map<K1, T1, C1, A1>& lhs,
...
class multimap {
 ...
 private:
  using rb_tree_type =
      toystl::rb_tree<key_type, value_type, toystl::identity<value_type>,
                      Compare, Allocator>;
  rb_tree_type tree_;  // 采用红黑树来实现
...
template <class K1, class T1, class C1, class A1>
bool operator==(const multimap<K1, T1, C1, A1>& lhs,
                const multimap<K1, T1, C1, A1>& rhs) {
  return lhs.size() == rhs.size() &&
         toystl::equal(lhs.cbegin(), lhs.cend(), rhs.cbegin());
}
