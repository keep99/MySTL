#ifndef TOYSTL_SRC_ITERATOR_INSERT_ITERATOR_H_
#define TOYSTL_SRC_ITERATOR_INSERT_ITERATOR_H_
...
// 此类型迭代器的底层实现需要调用指定容器的 push_back()
// 成员方法，这就意味着，此类型迭代器并不适用于 STL
// 标准库中所有的容器，它只适用于包含 push_back() 成员方法的容器。
// C++ STL 标准库中，提供有 push_back() 成员方法的容器包括 vector、deque 和
// list。
template <class Container>
class back_insert_iterator
    : public iterator_base<output_iterator_tag, void, void, void, void> {
 public:
  // 继承获得类型 iterator_category 、 value_type 、 difference_type 、 pointer
  // 及 reference
 ...
 public:
  ...
  // C++ Primer P610
  // 重载版本一般是两个，一个版本接受一个指向const的左值引用，另外一个版本接受一个指向非const的右值引用
  // //
...
// 那么与之对应的，front_insert_iterator 只适用于包含 push_front()
// 成员方法的容器。
template <class Container>
...
#endif  // TOYSTL_SRC_ITERATOR_INSERT_ITERATOR_H_