#ifndef TOYSTL_SRC_LIST_H_
#define TOYSTL_SRC_LIST_H_
...
class list {
 ...
 private:
  // allocator 的 rebind。list 希望能按照 T 一样的策略 来分配 list_node<T>
  // 类型的对象。
  // rebind实现了对不同类型使用同一种内存分配策略的要求。具体参考https://bbs.csdn.net/topics/200079053
  using list_node_allocator =
      typename Allocator::template rebind<detail::list_node<T>>::
          other;  // 这里的 template 是为了说明后面跟的是个模板，用法同 typename

  // 上面也可以写成,参考https://bbs.csdn.net/topics/110079160
  // using list_node_allocator = typename
  // Allocator::rebind<detail::list_node<T>>::other;
 ...
 public:
  ...
  allocator_type get_allocator() const { return list_node_allocator(); }
  ...
  iterator begin() {
    // list_iterator<T, T&, T*> 和 list_node<T> 都是只有一个成员：list_type
    // node_;
    // 所以可以互相转换
  ...
  iterator insert(const_iterator position, InputIterator first,
                  InputIterator last) {
    return insert_range_aux(position, first, last,
  ...
  void push_back(T&& value) { insert(end(), toystl::move(value)); }
  ...
  void push_front(T&& value) { insert(begin(), toystl::move(value)); }
  ...
  void swap(list& other) { toystl::swap(node_, other.node_); }
  ...
  /* C++ Primer P 587
   * 在类模板自己的作用域中，我们可以直接使用模板名而不提供实参。比如
   * 这里的形参类型，不是 list<T>, 而是直接写 list */
  void merge(list& other) { merge(other, toystl::less<T>()); }
  ...
  void remove(const_reference value) {
    remove_if([value](const_reference other) { return value == other; });
  }
   ...
   * @return {*}
   */
  void unique() { unique(toystl::equal_to<T>()); }
  ...
  void sort() { sort(begin(), end(), toystl::less<T>()); }
 ...
 private:
  ...
  link_type allocate_node() { return list_node_allocator::allocate(1); }
...
template <class T, class Allocator>
template <class InputIterator>
void list<T, Allocator>::assign_dispatch(InputIterator first2,
                                         InputIterator last2, false_type) {
  iterator first1 = begin();
  iterator last1 = end();
...
// 参考 SGI STL 的实现
// 由于 STL 本身的排序算法 sort 接受的是随机访问迭代器，但是双向链表 list
// 的迭代器是双向迭代器，因此，不能使用 STL 本身的排序算法 sort ，必须得自己定义
/**
 * @description: 对链表进行归并排序
 * @param  {*}
 * @return {*}
...
template <class T, class Allocator>
template <class Y>
typename list<T, Allocator>::iterator list<T, Allocator>::insert_aux(
    const_iterator position, Y&& value) {
  // C++ Primer P614 :
...
template <class T, class Allocator>
template <class InputIterator>
typename list<T, Allocator>::iterator list<T, Allocator>::insert_range_aux(
...
bool operator<(const list<T, Allocator>& left,
               const list<T, Allocator>& right) {
  return toystl::lexicographical_compare(left.cbegin(), left.end(),
