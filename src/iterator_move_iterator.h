#ifndef TOYSTL_SRC_ITERATOR_MOVE_ITERATOR_H_
#define TOYSTL_SRC_ITERATOR_MOVE_ITERATOR_H_
...
class move_iterator {
 ...
 public:
  ...
  template <class U>
  explicit move_iterator(const move_iterator<U>& other)
  ...
  // 返回当前元素的右值引用
  reference operator*() { return toystl::move(*current_); }

  pointer operator->() const { return base(); }

  // C++ Primer P605 尾置返回类型
  auto operator[](difference_type n) const
      -> decltype(toystl::move(current_[n])) {
...
#endif  //  TOYSTL_SRC_ITERATOR_MOVE_ITERATOR_H_
