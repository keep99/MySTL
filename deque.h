...
namespace MySTL {
    namespace detail {
        ...
        inline size_t __deque_buf_size(size_t T_size) {
            return (T_size < 512) ? static_cast<size_t>(512 / T_size) : static_cast<size_t>(1);
        }
        ...
        struct deque_iterator {
            ...
            deque_iterator() : current_(nullptr), first_(nullptr), last_(nullptr), node_(nullptr) {}

            deque_iterator(pointer v, map_pointer n) : current_(v), first_(*n), last_(*n + deque_buf_size()), node_(n) {}

            deque_iterator(const iterator& other) : current_(other.current_), first_(other.first_), last_(other.last_), node_(other.node_) {}

            deque_iterator(iterator&& rhs) noexcept 
                : current_(rhs.current_), first_(rhs.first_), last_(rhs.last_), node_(rhs.node_)
            {
            ...
            self& operator+=(difference_type n) {
                ...
                if (offset >= 0 && offset < deque_buf_size()) {
                    // 目标在同一缓冲区内
                    current_ += n;
                }
                    ...
                    difference_type nodeOffset = offset > 0 
                        ? (offset / static_cast<difference_type>deque_buf_size()) 
                        : -static_cast<difference_type>((-offset - 1) / deque_buf_size()) - 1;
                    // 切换至正确的节点（亦即缓冲区）
                    set_node(node_ + nodeOffset);
                    // 切换至正确的元素
                    current_ = first_ + (offset - nodeOffset * static_cast<difference_type>(deque_buf_size()));
                }
            ...
            difference_type operator-(const self& x) const {
                return static_cast<difference_type>(deque_buf_size()) * (node_ - x.node_ - 1) + (current_ - first_) + (x.last_ - x.current_);
            }

            // 重载比较操作
            bool operator==(const self& rhs) const { return current_ == rhs.current_; }
            bool operator<(const self& rhs) const { return node_ == rhs.node_ ? (current_ < rhs.current_) : (node_ < rhs.node_); }
    ...
    private:
        ...
        // 中控器
        T** map_;                       // 指向 map, map 是块连续区间，其中每个元素都是一个指针，指向一个节点(缓冲区)
    ...
    public:
        ...
        deque& operator=(const deque& other) {
            if (this != &other) {
                ...
                if (length >= other.size()) {
                    ...
                    auto newEnd = MySTL::copy(other.bein(), other.end(), begin());
                    erase(newEnd, end());
                } else {
                    // 当前 deque 比 other 小
                    // 则将 other 的一部分拷贝到当前的 deque，然后将剩下的元素插入到 cend() 之前
                    MySTL::copy_n(other.begin(), length, begin());
                    insert(finish_, other.begin() + static_cast<difference_type>(length), other.end());
                }
            }
        ...
        void clear() {
            ...
            for(map_pointer node = start_.node_ + 1; node < finish_.node_; ++node) {
                node_allocator::destroy(*node, deque_buf_size());
            }
        ...
        iterator erase(iterator first, iterator last) {
            if (first == begin() && last == end()) { // 如果清除区间就是整个 deque
                ...
                if (elementBefore < elementAfter) { 
                    ...
                    node_allocator::destroy(begin(), newStart);
                    ...
                    node_allocator::destroy(newFinish, end());
        ...
        void push_back(const value_types&& value) {
            emplace_back(MySTL::move(value));
        }
        ...
        void emplace_back(Args&&... args) {
            if (finish_.current_ != finish_.last_ - 1) {
                node_allocator::construct(finish_.current_, tinySTL::forward<Args>(args)...);
                ++finish_.current_;
            } else {
                // 最后一个缓冲区只剩下一个可用空间，则在尾部分配一个新的缓冲区。
                reserve_map_at_back();
        ...
        void pop_front() {
            if (start_.current_ != start_.last_ - 1) {
                ...
                start_.current_ = start_.first_;            // 使 current_ 指向下一个缓冲区的第一个元素
            }
        }
    ...
    private:
        ...
        void create_map_and_nodes(size_type numElements = 0) {
            ...
            try
            {
                for(cur = nStart; cur <= nFinish; ++cur) {
                    *it = node_allocator::allocate(deque_buf_size());
                }
            }
            catch (...)
            {
        ...
        /* 回收 map 的内存空间  */
        void deallocate_map() {
            map_allocator::deallocate(map_, mapSize_);
        }
        ...
        iterator reserve_elements_at_back(size_type n) {
            size_type nAfter = finish_.last - finish_.current_ - 1;
            if(n > nAfter) {
                // 空间不够，需要分配新的缓冲区
        ...
        void reallocate_map(size_type nodesToAdd, bool addToFront) {
            ...
            T** newStart;
            if (mapSize_ > 2 * newNumNodes) { // 现在 map 有足够的位置，所以只需移动元素即可
                ...
                if (newStart < start_.node_) {
                    ...
                    MySTL::copy_backward(start_.node_, finish_.node_ + 1, newStart + nodesToAdd);
                }
            } 
            else { // 如果增加缓冲区后的个数的 2 倍大于等于 mapSize_，则需要重新配置空间。此时，会导致所有迭代器失效。
                mapSize_ += MySTL::max(mapSize_, nodesToAdd) + 2;
