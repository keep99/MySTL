#ifndef TOYSTL_TEST_TEST_DEQUE_H_
#define TOYSTL_TEST_TEST_DEQUE_H_
...
class TestDeque : public ::testing::Test {
 protected:
  ...
  void ExpectEqual() const {
    EXPECT_EQ(abc_deque_of_kitten.empty(), std_deque_of_kitten.empty());
    EXPECT_EQ(abc_deque_of_kitten.size(), std_deque_of_kitten.size());
    for (int i = 0; i < static_cast<int>(abc_deque_of_kitten.size()); i++) {
      EXPECT_EQ(abc_deque_of_kitten[i], std_deque_of_kitten[i]) << i;
    }
  }
};

TEST_F(TestDeque, ConstructorDefault) { ExpectEqual(); }
...
TEST_F(TestDeque, FrontAndBack) {
  for (const auto& i : std_deque_of_id) {
    std_deque_of_kitten.emplace_back(i);
    abc_deque_of_kitten.emplace_back(i);
    EXPECT_EQ(abc_deque_of_kitten.front(), std_deque_of_kitten.front());
    EXPECT_EQ(abc_deque_of_kitten.back(), std_deque_of_kitten.back());
  }
}
...
TEST_F(TestDeque, At) {
  ...
  for (const auto& i : std_deque_of_id) {
    std_deque_of_kitten.emplace_back(i);
    abc_deque_of_kitten.emplace_back(i);
    EXPECT_EQ(abc_deque_of_kitten.at(j), std_deque_of_kitten.at(j));
    j++;
  }
}
...
#endif  // TOYSTL_TEST_TEST_DEQUE_H_